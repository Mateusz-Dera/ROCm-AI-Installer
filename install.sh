...
# Save configuration
save_config() {
    # Use fallback values if display variables are not set
...
create_container() {
    ...
    if whiptail --title "Create Container" --yesno "This will:\n1. Stop and remove existing container\n2. Build new container (podman-compose build)\n3. Start container (podman-compose up -d)\n\nContinue?" 14 60 2>&1 > /dev/tty; then
        ...
        # Allow local X11 connections for GUI applications
...
text_generation() {
    ...
    while $second; do
        ...
        choice=$(whiptail --title "Text generation" --menu "Choose an option:" 16 100 8 --cancel-button "Back" \
            1 "Install KoboldCPP" \
            2 "TabbyAPI" \
            3 "SillyTavern" \
            4 "Install llama.cpp" \
            5 "Install llama.cpp Vulkan" \
            6 "Install turboquant-rocm-llamacpp" \
            7 "Install vllm-turboquant" \
            8 "hipfire" \
            2>&1 > /dev/tty)
        status=$?
