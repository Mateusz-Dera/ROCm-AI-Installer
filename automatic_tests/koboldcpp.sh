...
test_koboldcpp() {
    ...
    podman exec -t rocm bash -c "pkill -f '[k]oboldcpp' 2>/dev/null; sleep 1; : > '${kobold_log}'" || true
    ...
    podman exec -t rocm bash -c "pkill -f '[k]oboldcpp' 2>/dev/null || true" || true
    local kw=0
    while podman exec -t rocm bash -c "pgrep -f '[k]oboldcpp' > /dev/null" 2>/dev/null; do
