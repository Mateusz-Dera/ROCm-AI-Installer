...
phase4_verify_llama() {
    ...
    podman exec -t rocm bash -c "pkill -f 'llama-server' 2>/dev/null; sleep 1; : > /tmp/llama_server.log" || true
    ...
    # Monitors log for "llama server listening" as early-ready hint;
    # also detects crashes immediately instead of waiting out the full timeout.
    ...
    if [ $wait_rc -eq 0 ]; then
        pass "llama.cpp server is running and /health responded OK"
    else
        podman exec -t rocm bash -c "cat /tmp/llama_server.log" 2>/dev/null || true
    ...
    if echo "$api_response" | grep -q '"content"'; then
        ...
        podman exec -t rocm bash -c "cat /tmp/llama_server.log" 2>/dev/null || true
        abort "llama.cpp API did not return expected response (missing 'content' field)"
    fi
