...
phase4_verify_llama() {
    ...
    if echo "$api_response" | grep -q '"choices"'; then
        ...
        if echo "$answer" | grep -q '4'; then
            pass "llama.cpp API responded correctly (answer contains 4)"
        else
            abort "llama.cpp API returned wrong answer: \"$answer\" (expected 4)"
        fi
    else
        info "Raw API response: $api_response"
        podman exec -t rocm bash -c "cat '${server_log}'" 2>/dev/null || true
        abort "llama.cpp API did not return expected response"
    fi
