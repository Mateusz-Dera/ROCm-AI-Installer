...
install_sam_3d_objects(){
    ...
    # --override pins utils3d to our commit (moge pulls a conflicting commit)
    podman exec -it rocm bash -c "cd /AI/$FOLDER && source .venv/bin/activate && uv pip install hatchling hatch-requirements-txt editables && uv pip install -e . --no-build-isolation --override requirements.txt"
    ...
    podman cp "$SCRIPT_DIR/custom_files/sam-3d-objects/app_gradio.py" "rocm:/AI/$FOLDER/app_gradio.py"

    # Persist HF_TOKEN inside the container so all apps can use it
    if [ -n "$HF_TOKEN" ]; then
        podman exec -it rocm bash -c "
            grep -v '^HF_TOKEN=' /etc/environment > /tmp/env_tmp 2>/dev/null || true
            echo 'HF_TOKEN=$HF_TOKEN' >> /tmp/env_tmp
            mv /tmp/env_tmp /etc/environment"
    fi

    # Download checkpoints from HuggingFace if HF_TOKEN is set.
    # Access must be requested first at https://huggingface.co/facebook/sam-3d-objects
    if [ -n "$HF_TOKEN" ]; then
        podman exec -it rocm bash -c "cd /AI/$FOLDER && source .venv/bin/activate && \
            huggingface-cli download \
                --repo-type model \
                --token $HF_TOKEN \
                --local-dir checkpoints/hf-dl \
                --max-workers 1 \
                facebook/sam-3d-objects && \
            mv checkpoints/hf-dl/checkpoints checkpoints/hf && \
            rm -rf checkpoints/hf-dl"
    else
        echo "HF_TOKEN not set – skipping checkpoint download."
