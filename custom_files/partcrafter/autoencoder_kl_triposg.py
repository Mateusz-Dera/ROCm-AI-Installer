...
def fps(src: _torch.Tensor, batch: _Optional[_torch.Tensor] = None,
        ratio: float = 0.5, random_start: bool = True,
        batch_size: _Optional[int] = None) -> _torch.Tensor:
    device = src.device
    if batch is None:
        batch = _torch.zeros(src.size(0), dtype=_torch.long, device=device)
    _bs = int(batch.max().item()) + 1 if batch_size is None else batch_size
    deg = src.new_zeros(_bs, dtype=_torch.long)
    deg.scatter_add_(0, batch, _torch.ones_like(batch))
    ptr = deg.new_zeros(_bs + 1)
    _torch.cumsum(deg, 0, out=ptr[1:])
    result = []
    for b in range(_bs):
        ...
        pts = src[s:e, :3].float()
        chosen = [_torch.randint(0, n, (1,)).item() if random_start else 0]
        min_dists = ((pts - pts[chosen[-1]]) ** 2).sum(-1)
        for _ in range(k - 1):
            idx = int(min_dists.argmax().item())
            chosen.append(idx)
            dists = ((pts - pts[idx]) ** 2).sum(-1)
            min_dists = _torch.minimum(min_dists, dists)
        result.append(_torch.tensor(chosen, dtype=_torch.long, device=device) + s)
    return _torch.cat(result) if result else _torch.empty(0, dtype=_torch.long, device=device)
